...
const EditProfile = () => {
  ...
  useEffect(() => {
    if(access_token) {
      ...
      { username: userAuth.username }).then(({ data }) => {
        setProfile(data);
        setLoading(false);
      })
      .catch(err => {
            ...
            <div className="flex flex-col items-start gap-8 py-10 lg:flex-row lg:gap-10">
              <div className="mb-5 max-lg:center">
                <label 
                  htmlFor="uploadImg" 
                  id="profileImgLabel"
                  className="relative block w-48 h-48 mx-auto overflow-hidden rounded-full bg-grey"
                >
                  <div className="absolute top-0 left-0 flex items-center justify-center w-full h-full text-white opacity-0 cursor-pointer bg-black/30 hover:opacity-100">
                    <i className="mr-1 fi fi-br-picture" /> 
                    Upload image
                  </div>
                  <img 
                    src={profile_img}
              ...
              <div className="w-full">
                <div className="grid grid-cols-1 md:grid-cols-2 md:gap-5">
                  <div>
                    <InputBox name="fullname" type="text" value={fullname} placeholder="Full Name" disabled={true} icon="fi fi-br-user" />
                  </div>
                  <div>
                    <InputBox name="email" type="email" value={email} placeholder="Email" disabled={true} icon="fi fi-br-envelope" />
                  </div>
                </div>
                <InputBox type="text" name="username" value={profile_username} placeholder="Username" icon="fi fi-br-at" />
